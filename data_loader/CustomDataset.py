...
class DataLoader(DataLoaderBase):
    ...
    def create_social_graph(self):
        self.social_graph=torch.zeros(self.n_users,self.n_users)
        for key in self.social_dict.keys():
            self.social_graph[key,self.social_dict[key]]=1
        for key in self.train_kg_dict.keys():
            user_list = []
            if key<self.n_items:
                for (t,r) in self.train_kg_dict[key]:
                    if t>self.n_entities:
                        user_list.append(t)
            for u in user_list:
                self.social_graph[u, user_list] = 1
