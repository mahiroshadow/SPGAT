...
class DataLoaderBase(object):
    ...
    def __init__(self, args):
        ...
        self.social_dict=self.load_social_dict(self.social_file)
    ...
    def load_social_dict(self,filename):
        social_data=pd.read_csv(filename,sep=' ',names=['u1','u2'],dtype=int)
        social_dict=collections.defaultdict(list)
        for idx in range(len(social_data)):
            social_dict[social_data["u1"][idx]].append(social_data["u2"][idx])
        return social_dict
    ...
    def sample_neg_triples_for_h(self, kg_dict, head, relation, n_sample_neg_triples, highest_neg_idx):
        ...
        while True:
            ...
            if (tail, relation) not in pos_triples and tail not in sample_neg_tails:
                ...
                sample_neg_rels.append(relation)
        return sample_neg_tails,sample_neg_rels
    ...
    def generate_kg_batch(self, kg_dict, batch_size, highest_neg_idx):
        ...
        batch_relation, batch_pos_tail, batch_neg_tail ,batch_neg_rel= [], [], [],[]
        for h in batch_head:
            relation, pos_tail = self.sample_pos_triples_for_h(kg_dict, h, 1)
            batch_relation += relation
            batch_pos_tail += pos_tail

            neg_tail ,neg_rel= self.sample_neg_triples_for_h(kg_dict, h, relation[0], 1, highest_neg_idx)
            batch_neg_tail += neg_tail
            batch_neg_rel+=neg_rel

        batch_head = torch.LongTensor(batch_head)
        batch_relation = torch.LongTensor(batch_relation)
        batch_pos_tail = torch.LongTensor(batch_pos_tail)
        batch_neg_tail = torch.LongTensor(batch_neg_tail)
        batch_neg_rel=torch.LongTensor(batch_neg_rel)
        return batch_head, batch_relation, batch_pos_tail, batch_neg_tail,batch_neg_rel
